...
export default function QuizBuilderScreenCreate({ route, navigation }) {
    const { totalQuestions, QuizTitle } = route.params;
    ...
    const handleNext = () => {
        if (!questionText.trim() || options.some(opt => !opt.trim()) || correctIndex === null) {
            Alert.alert('Please fill out the question, all options, and select a correct answer.');
            return;
        }
        ...
        if (currentIndex + 1 >= totalQuestions) {
            // All questions done
            navigation.navigate('QuizBuilderPreview', { questions: updatedQuestions, quizTitle: QuizTitle });
        } else {
    ...
    return (
        ...
        <Layout style={styles.container}>
            <ScrollView contentContainerStyle={styles.content}>
                <Text category="h5" style={{ marginBottom: 16, textAlign:'Center', alignSelf:'center'}}>{QuizTitle || 'Create Your Quiz'}</Text>
                <Text category="h6">Question {currentIndex + 1} of {totalQuestions}</Text>
